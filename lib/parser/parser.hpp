...
struct HittableObjectVisitor
    : public boost::static_visitor<std::shared_ptr<hittable>> {
    std::shared_ptr<hittable> operator()(const Sphere& s) const {
        Eigen::Vector3f center(s.center.x, s.center.y, s.center.z);
        return std::make_shared<HittableSphere<float>>(center, s.radius);
    }
    std::shared_ptr<hittable> operator()(const Quadrangle& q) const {
        ...
        return std::make_shared<HittableQuad<float>>(p1, p2, p3, p4);
    }
    std::shared_ptr<hittable> operator()(const Box& b) const {
        ...
        return std::make_shared<HittableTriangle<float>>(p1, p2, p3);
    }
    std::shared_ptr<hittable> operator()(const Triangle& t) const {
...
class Parser {
   ...
   public:
    ...
    std::vector<std::shared_ptr<hittable>> operator()();

   private:
    std::shared_ptr<hittable> parse_object(const Object& object);
};
