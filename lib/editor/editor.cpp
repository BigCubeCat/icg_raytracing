...
Editor::Editor(DataModel* data, Parser* parser, QWidget* parent)
    : QWidget(parent), m_data(data), m_parser(parser) {
...
void Editor::apply() {
    ...
    m_parser->set_objects_text(text);
    if (m_parser->is_valid()) {
        auto conf = (*m_parser)();
        m_data->set_config(conf);
        emit new_objects();
    }
}
