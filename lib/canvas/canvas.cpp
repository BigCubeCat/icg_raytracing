...
void Canvas::paintEvent(QPaintEvent* event) {
    ...
    make_view_matrix(m_view, m_point_cam, m_point_view, m_vec_up);
    make_projection_matrix(m_proj, m_zn, m_zf, m_sw, m_sh);
    make_rotation_matrix(m_rot, m_rotation_x, m_rotation_y);
    auto func = m_view * m_rot;
    ...
    for (const auto& v : fig) {
        double depth;
        auto new_point =
            project_point(v, func, m_proj, width(), height(), depth);
        ts.push_back(depth);
...
void Canvas::keyPressEvent(QKeyEvent* event) {
    ...
    QWidget::keyPressEvent(event);  // Передаем событие базовому классу
}
