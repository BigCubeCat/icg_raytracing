...
void make_view_matrix(Eigen::Matrix4d& dest, const Eigen::Vector3d& eye,
    ...
    dest = Eigen::Matrix4d::Identity();
    dest.block<1, 3>(0, 0) = s.transpose();
    dest.block<1, 3>(1, 0) = u.transpose();
    dest.block<1, 3>(2, 0) = -f.transpose();
    dest(0, 3) = -s.dot(eye);
    dest(1, 3) = -u.dot(eye);
    dest(2, 3) = f.dot(eye);
}
...
void make_projection_matrix(Eigen::Matrix4d& dest, double zn, double zf,
                            double sw, double sh) {
    dest = Eigen::Matrix4d::Zero();
    dest(0, 0) = 2 * zn / sw;
    dest(1, 1) = 2 * zn / sh;
    dest(2, 2) = -(zf + zn) / (zf - zn);
    dest(2, 3) = -2 * zf * zn / (zf - zn);
    dest(3, 2) = -1;
}
